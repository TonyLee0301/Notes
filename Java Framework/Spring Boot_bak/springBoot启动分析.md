...
|ParentContextCloserApplicationListener|ParentContextAvailableEvent| 当父上下文可用时触发 ，该监听器在父上下文可用是会创建一个 关闭当前上下文的监听器，并将该监听器注册到父上下文中，当父上下文关闭时，该监听器会传递关闭当前上下文。这个 listener 就是为了构建一个层级监听|
...
可以看出，spring boot 启动过程中各个阶段，都会有相关的事件需要对应的listener处理。每个listener都指负责单一的工作。虽然可能监听的事件有多个，但是处理的事情也是同一个工作。这也符合面向对象中`单一职责`的原则。
